...
function monthlyDueDate(anchorDate, dueDay) {
  const source = new Date(anchorDate);
  if (Number.isNaN(source.getTime()) && !dueDay) return null;

  const today = new Date();
  today.setHours(0, 0, 0, 0);
  const day = Number(dueDay || source.getDate());
  const candidate = new Date(today.getFullYear(), today.getMonth(), Math.max(1, Math.min(31, day)));
  if (candidate.getMonth() !== today.getMonth()) candidate.setDate(0);
  candidate.setHours(0, 0, 0, 0);

  if (candidate < today) {
    const next = new Date(today.getFullYear(), today.getMonth() + 1, Math.max(1, Math.min(31, day)));
    if (next.getMonth() !== ((today.getMonth() + 1) % 12)) next.setDate(0);
...
async function buildReminderItems(prisma, tenantKey) {
  ...
  for (const loan of loans) {
    addReminder(items, {
      ...
      dueDate: monthlyDueDate(loan.nextDueDate, loan.dueDay),
