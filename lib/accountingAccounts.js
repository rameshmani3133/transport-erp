...
async function ensureStandardAccountingAccounts(tx, req) {
  const definitions = [
    ...
    ['Round Off', { accountType: 'Income', accountGroup: 'Indirect Income', balanceType: 'Cr' }],
  ...
  const accountNames = definitions.map(([accountName]) => accountName);
  const existing = await tx.account.findMany({
    where: withTenant(req, { accountName: { in: accountNames } })
  });
  const existingNames = new Set(existing.map(account => account.accountName));
  const missing = definitions
    .filter(([accountName]) => !existingNames.has(accountName))
  ...
  if (missing.length) {
    await tx.account.createMany({ data: missing, skipDuplicates: true });
  }

  const accounts = missing.length
    ? await tx.account.findMany({ where: withTenant(req, { accountName: { in: accountNames } }) })
    : existing;
  return Object.fromEntries(accounts.map(account => [account.accountName, account]));
}
