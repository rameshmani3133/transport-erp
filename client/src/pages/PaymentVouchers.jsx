...
export default function PaymentVouchers() {
  ...
  const columns=[
    ...
    {header:'Actions',key:'actions',render:v=>v.status==='Posted'&&v.voucherType!=='REVERSAL'?<select aria-label={`Actions for ${v.voucherNo}`} value="" onChange={e=>handleVoucherAction(v,e.target.value)} style={{padding:'7px 28px 7px 10px',border:'1px solid #cbd5e1',borderRadius:'6px',background:'#fff',color:'#334155',fontWeight:700,cursor:'pointer',minWidth:'105px'}}><option value="" disabled>Actions</option><option value="edit">Edit</option><option value="reverse">Reverse</option><option value="void">Delete / Void</option></select>:<span style={{color:'#64748b',fontSize:'12px'}}>{v.reversalOf?.voucherNo?`For ${v.reversalOf.voucherNo}`:'No actions'}</span>}
  ];
